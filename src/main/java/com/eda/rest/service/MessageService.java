...
public class MessageService implements IMessageService {
	...
	//@Autowired
	public SequenceDao sequenceDao = new SequenceDao();
	//@Autowired
	...
	public JSONObject createMessage(JSONObject jsonMessage) {
		...
		String responseClassification = null;
		...
		try
		{
			...
			String event_id=jsonMessage.optString("event_id");
			conversation_id=jsonMessage.optString("conversation_id");
			client_id=jsonMessage.optString("client_id");
			...
			else if(jsonMessage.getLong("thread_ref_id") == 0L && (jsonMessage.getString("direction").equals("O"))){
				returnToApp.put("error","empty thread");
				return returnToApp;
			}
			...
			/*Classification classification = service.classify("0235B6x12-nlc-407",
						msg_content);*/
			/*Classification classification = service.classify("3AE103x13-nlc-1491",
						msg_content);*/
			Classification classification = service.classify("3B015Fx14-nlc-2017",
						msg_content);
			logger.info("WATSON ANSWER:::::"+classification);
			Message.setNlc_classification(classification.getClasses().toString());
			responseClassification = classification.getClasses().get(0).getName().toString();
			logger.info("WATSON ANSWER:::::"+responseClassification);
			...
			returnToApp.put("device_id",device_id);
			returnToApp.put("event_id", event_id);
			if(conversation_id.equals("") && client_id.equals("")){
			returnToApp.put("conversation_id", "1");
			returnToApp.put("client_id", "0");
			}else{
				returnToApp.put("conversation_id", conversation_id);
				returnToApp.put("client_id", client_id);
			}
			returnToApp.put("classification", responseClassification);
