...
public abstract class AbstractController {
	...
	static JSONObject readJSONData(HttpServletRequest request) {
		...
		try {
			...
			InputStream in = request.getInputStream(); // Get the data in the
														// entity
	...
	public Result sendGcmPushNotification(Long thread_ref_id, String msg_content) throws IOException {
		 ...
		 */
		String regId = messageService.getRegKeyForParticularThread(thread_ref_id);

		String userMessage = msg_content;// "TEST MESSAGE FROM WEB SERVICE";
		Sender sender = new Sender(GOOGLE_SERVER_KEY);

		Message message = new Message.Builder().timeToLive(30).delayWhileIdle(true).addData(MESSAGE_KEY, userMessage)
				.build();
	 ...
	 * Message.setSender_id(1L); Message.setDevice_id(device_id);
	 * Message.setMsg_content(status); Message.setOrigin_ip("");
	 * Message.setTime_stamp(new Date()); Message.setThread_ref_id(thread_id);
	 * Message.setParent_id(1L); Message.setAgent_id(1L);
	 * Message.setDirection("I"); Message.setPath("path");
	 * 
	 * return Message; }
	 */
